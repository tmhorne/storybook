...
public class CharactersLocationsPanel extends JPanel implements MouseListener,
		IRefreshable {
	...
	public CharactersLocationsPanel(Scene scene) {
		...
		characterFactory = new CharacterLinksFactory(
				characterLinksList, scene);
		locationFactory = new LocationLinksFactory(
				locationLinksList, scene);
		initGUI();
	}
	...
	private void initGUI() {
		...
		// chosen characters
		JLabel lbCharacterLink = new JLabel(
				I18N.getMsgColon("msg.dlg.scene.person.links"));
		chosenCharacters = characterFactory.createList(
				scene,
				scene == null ? true : false);
		...
		// add character
		IconButton btAddCharacter = new IconButton(
				"icon.small.arrow.up",
				"msg.common.add",
				getAddCharacterAction());
		
		// remove character
		IconButton btRemoveCharacter = new IconButton(
				"icon.small.arrow.down",
				"msg.common.delete",
				getRemoveCharacterAction());

		// new character
		IconButton btNewCharacter = new IconButton(
				"icon.medium.new.person",
				"msg.common.person.new",
				getNewCharacterAction());
		...
		JLabel lbLocation = new JLabel(
				I18N.getMsgColon("msg.dlg.scene.location.links"));
		chosenLocations = locationFactory.createList(
				scene,
				scene == null ? true : false);
		...
		// add location
		IconButton btAddLoction = new IconButton(
				"icon.small.arrow.up",
				"msg.common.add",
				getAddLocationAction());
		
		// remove location
		IconButton btRemoveLoction = new IconButton(
				"icon.small.arrow.down",
				"msg.common.delete",
				getRemoveLocationAction());
		
		// new location
		IconButton btNewLocation = new IconButton(
				"icon.medium.new.location",
				"msg.common.location.new",
				getNewLocationAction());
	...
	private AbstractAction getNewCharacterAction(){
		return new AbstractAction(){
			@Override
			public void actionPerformed(ActionEvent e) {
				ActionRegistry.getInstance().getAction(
						SbAction.CHARACTER_NEW).actionPerformed(null);
				refresh();
			}
		};
	}
	...
	private AbstractAction getNewLocationAction(){
		return new AbstractAction(){
			...
			public void actionPerformed(ActionEvent e) {
				ActionRegistry.getInstance().getAction(
						SbAction.LOCATION_NEW).actionPerformed(null);
				refresh();
			}
		};
	}

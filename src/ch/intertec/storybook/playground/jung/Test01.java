...
public class Test01 extends JungTest {
	/**
	 * the graph
	 */
	 ...
	 */
	String[] iconNames = { "apple", "os", "x", "linux", "inputdevices",
			"wireless", "graphics3", "gamespcgames", "humor", "music",
			"privacy" };
	...
	public Test01(Container content){
		...
		for (int i = 0; i < vertices.length; i++) {
			...
			try {
				Icon icon = new LayeredIcon(new ImageIcon(
						Test01.class.getResource(name)).getImage());
				iconMap.put(vertices[i], icon);
			} catch (Exception ex) {
				System.err
						.println("You need slashdoticons.jar in your classpath to see the image "
								+ name);
			}
		}
		...
		Transformer<Number, Paint> vpf = new PickableVertexPaintTransformer<Number>(
...
//		Container content = getContentPane();
	...
	/**
	 * A simple implementation of VertexStringer that gets Vertex labels from a
	 * Map
	 * 
	 * @author Tom Nelson
	 * 

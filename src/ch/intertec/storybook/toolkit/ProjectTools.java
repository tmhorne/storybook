...
public class ProjectTools {
	...
	public static void createDemoFile() {
		try {
			...
			demo.createObjects();
			ActionRegistry.getInstance().getAction(SbAction.REFRESH)
		...
		if (fileName.exists() || fileNameOld.exists()) {
			if (showWarning) {
				JOptionPane.showMessageDialog(MainFrame.getInstance(),
						I18N.getMsg("msg.prjtools.project.exits.text", file),
						I18N.getMsg("msg.prjtools.project.exits.title"),
		...
		try {
			if (filename != null) {
				PrefManager.getInstance().setValue(
						Constants.Preference.LAST_OPENED_FILE, filename);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private static void updateRecentFiles(File file) {
		try {
			String csFiles = PrefManager.getInstance().getStringValue(
					Preference.RECENT_FILES);
			...
			// logger.debug(PrefManager.toCsString(files));
			PrefManager.getInstance().setValue(Preference.RECENT_FILES,
					PrefManager.toCsString(files));
		} catch (Exception e) {
			logger.error(e.getMessage());
		}
	}
	...
	public static File initProjectDir(File dir) {
		if (projectDir == null) {
			projectDir = new File(dir + File.separator + ".storybook"
					+ File.separator + Constants.ProjectDirectory.PROJECTS);
			projectDir.mkdir();
		}
		return projectDir;
	}
	...
	private static boolean createFile(File file, boolean isDemo) {
		...
		try {
			// if another project is opened, close it
	...
	private static void setup(File file) {
		try {
			PrefManager.getInstance().setValue(Preference.LAST_OPENED_DIRECTORY,
					FilenameUtils.getFullPath(file.toString()));			
			PrefManager.getInstance().setValue(Preference.LAST_OPENED_FILE,
		...
		PCSDispatcher.getInstance().firePropertyChange(
				PCSDispatcher.Property.PROJECT.toString(), null, true);
	}

	public static void saveView() {
		try {
			Internal internal = InternalPeer
					.doSelectByKey(Constants.ProjectSetting.VIEW);
			if (internal == null) {
	...
	private static void savePartId() {
		try {
			...
			Internal internal = InternalPeer
					.doSelectByKey(Constants.ProjectSetting.PART_ID);
			if (internal == null) {
				internal = new Internal();
			}
	...
	public static void closeFile() {
		...
		PCSDispatcher.getInstance().firePropertyChange(
				PCSDispatcher.Property.PROJECT.toString(), true, null);
